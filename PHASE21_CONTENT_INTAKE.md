...
- 先保留原始链接，再生成一份标准化链接
- 标准化时会清掉常见追踪参数，例如 `utm_*`、`fbclid`、`gclid`、`si`
- 会去掉 fragment，并统一 hostname 的常见前缀，例如 `www.`、`m.`
- 正式生成时，先按原始链接精确匹配，再按标准化链接匹配
- 如果数据库里旧来源的链接格式不一致，会再做一轮“取出同平台来源后标准化比较”
- 找到已存在来源时，复用该 `source`，不重复新建
- 没找到时，才创建新的 `source`
...
- 本次查重使用的候选链接
- 预期是复用还是新建
- 如果能找到已存在来源，会把来源信息一起写进报告

正式生成报告现在会额外输出：

- 来源是 `created` 还是 `reused`
- 本次命中的匹配类型：
  - `exact`
  - `normalized_exact`
  - `normalized_compare`
...
- 暂不做 Directus 自定义模块
- 暂不改成 Next.js 独立运营后台
- 先继续使用 `content_intake` 单表统一上传台

推荐原因：

- 统一上传台已经把原来分散的录入动作收成一个入口
- 表单顺序、发布方式、重复来源检测、验证脚本、运营手册都已经补齐
- 现在真正缺的不是一个新的后台外壳，而是先用真实内容验证这套录入方式是否顺手

### 19.2 三个可选方案

| 方案 | 适合什么情况 | 优点 | 问题 |
|---|---|---|---|
| 继续用 `content_intake` 单表 | 目前这个阶段 | 改动最小，运营可以马上开始用 | 交互体验仍然是 Directus 原生表单 |
| 做 Directus 自定义模块 | 统一上传台仍然不够顺手，但还想继续留在 Directus 里 | 还能共用现有权限、集合和后台能力 | 开发成本明显上升，维护也更重 |
| 改成 Next.js 独立运营后台 | 未来运营流程明显复杂，且需要更强交互 | 体验最好，可完全按运营流程设计 | 成本最高，要自己维护后台页面、权限和流程 |

### 19.3 当前推荐方案

当前推荐：

- 继续用 `content_intake` 单表

不推荐现在直接做更重后台，原因是：

- 还没有完成一轮真实内容录入验证
- 还没证明现在的表单真的扛不住日常使用
- 如果现在就做更重后台，容易把时间花在界面重做上，而不是先把实际内容录进去

### 19.4 什么时候再考虑升级

只有出现下面这些情况，再考虑进入下一阶段：

- 运营人员连续多次反馈“还是需要在多个地方来回跳”
- 单表表单已经无法承载实际录入流程
- 需要更强的批量导入、批量编辑、批量审核
- 需要文件预览、拖拽排序、分步骤录入、可视化发布流
- 需要比 Directus 默认表单更强的交互体验

### 19.5 下一阶段建议

当前 Phase 21 的结论不是继续造新后台，而是先做这两件事：

1. 用真实内容跑一轮统一上传台
2. 记录运营实际卡住的点

如果后面确认问题主要集中在表单交互，再优先考虑：

- Directus 自定义模块

只有当后台需求明显超过 Directus 能力时，再考虑：
